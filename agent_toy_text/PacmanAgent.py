...
class PacmanAgent(object):
    ...
    def __init__(self, env):
        ...
        self.env_size = 25 ################################################### Je nach Map anpassen
        ...
        self.state_shape = (self.env_size+1,)
        self.lr = 1e-3
    ...
    def state_presenter(self, state):
        # Spielfeld initialisieren in einer Spalte
        f_state = np.zeros(self.env_size)
        
        # In state sind die Positionen des Pacman und der Ghosts übergeben
        # Positionen werden in 1 dimensionalen Spielfeld eingefügt
        ...
        for i in range(1,5):
            if state[i] != 0:
                if i == 1:
                    f_state[state[i]] = -3 # HUNTER
                if i == 2:
                    f_state[state[i]] = -1 # RANDOM
                if i == 3:
                    f_state[state[i]] = -2 # EAGER
                if i == 4:
                    f_state[state[i]] = -1 # PASSIVE

        # Zahl der verbleibenden Dots anhängen
        f_state = np.append(f_state, state[5])
