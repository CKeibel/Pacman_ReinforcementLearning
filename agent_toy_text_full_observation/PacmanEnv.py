...
class PacmanEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.low = np.array([0 for i in range(26)], dtype=np.int8)
    self.high = np.array([9 for i in range(26)], dtype=np.int8)
  ...
  def step(self, action):
    ...
    ob = np.zeros(25) # länge*breite
    if self.percept is None:
         pass
    else:
        view = self.percept['view']
        for x in range(0, len(view)):
            for y in range(0, len(view[x])):
                # Wall
                if view[x][y] == 'W':
                    tmp = self.get_position(x, y)
                    ob[tmp] = 9
                # Pacman
                elif view[x][y] == 'P':
                    tmp = self.get_position(x, y)
                    ob[tmp] = 2
                # Ghosts and Ghost+Dot   
                elif view[x][y] == 'G' or view[x][y] == 'GD':
                    tmp = self.get_position(x, y)
                    ob[tmp] = -1
                # reachable Dots
                elif view[x][y] == 'D':
  ...
  def reset(self):
    ...
    return np.zeros(25) ################################################################ Inital State je nach Map anpassen
  ...
  def get_position(self, x, y):
    position = (len(self.percept['view']) * y) + (x)
